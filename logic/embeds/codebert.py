...
from logic.csvreader import read_labels, read_functions
...
def _remove_comments_and_docstrings(source, lang):
    ...
    temp = []
    for x in re.sub(pattern, _replacer, source).split('\n'):
        if x.strip() != "":
            temp.append(x)
    return '\n'.join(temp)
...
def build(code_path, file_name, batch_size=64):
    ...
    for i, j in enumerate(lines):
        examples.append(_InputExample(i, _remove_comments_and_docstrings(j, 'java'), label=labels_array[i]))
...
#     build("../../data/raw/unique_data_setV3.csv", "test")