...
from logic.augmentation.processing_source_code import *
...
def add_print(method_string):
    ...
    delete_index = [(i,i+1,i+2) for i in range(len(statement_list)) if 'for' in statement_list[i] or 'while' in statement_list[i]] 
    delete_index_list = [element[i] for element in delete_index for i in range(len(delete_index[0]))]
    statement_list = [statement_list[i] for i in range(len(statement_list)) if i not in delete_index_list]

    if(len(statement_list) - 2<=0):
        return method_string
    mutation_index = random.randint(1, len(statement_list) - 2)
    statement      = statement_list[mutation_index]
    new_statement  = '\t' + 'System.out.println("' + str(random.choice(word_synonym_replacement(statement)[0])) + '");'
